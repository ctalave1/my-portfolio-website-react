      ...
      description: [
        {
          ...
          responsibilities: [
            ...
            'Helped  found  and  develop  LoadNinja,  a  leading  load  testing  SaaS,  SPA  web platform,  from  the  ground  up.   Developed  using  React  (Class  Components, Functional Components, Contexts, React Router) on the front-end in conjunction with AWS (DynamoDB, Lambda, API Gateway, Cognito, EC2, etc) and Node.js on the back-end. Allows for the visual recording of load testing scripts, without writing code, and running load tests, all using real Chrome browsers.',
            'Developed  the  functionality  (Node.js,  Express,  React)  of  the  LoadNinja  recorder, running  on  an  EC2  instance,  that  allows  for  the  visual  recording  of  website  load testing scripts, generated as a Mocha script in real-time, and allowing for the visual replay  and  saving  of  the  recorder-generated  Mocha  script.  This  was  achieved  by exposing to the end user an active, graphical instance of a headless Chrome browser, shown via browser frames displayed in an iframe on the platform recorder UI,  which the user could then interact with and see, edit, save and replay their recorded actions on  a  website  using  the  recorder  sidebar.  This  was  achieved  by  using  Chrome DevTools Protocol (CDP) to interact with an the active Chrome browser, capturing the active  browser  frames,  sending  them  via WebSocket to the UI, and capturing user actions  on  the  active  browser  iframe  on  the  UI  and  sending  those  actions  via WebSocket  to  the  backend  recorder  to  be translated to browser actions executed using CDP.',
            'Developed  the  logic for correctly and equally distributing load test virtual users across multiple EC2 instances, spun up for a load test using the scripts, regions and proportions specified by the end user on the UI (React), via an AWS Lambda microservice  written in Node.js. The metrics and data returned by the load test scripts  running on EC2 were then shown visually on the platform’s (React) load test page via a combination of tabs, tables, and graphs (Highcharts).',
            'Developed  AWS  Lambda  microservice,  written  in  Node.js,  that  allows  for  the saving of a recording of a failed load test script iteration using FFMPEG as a layer in the Lambda function to piece together the gathered image frames of the script iteration playback into a single video which is then saved to S3 so that the user may view the video in their load test report on the React frontend, after the load test has completed.',
